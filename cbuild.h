...
public:
	...
	CBuild &moc_src(std::vector<std::string> src);
	...
	bool compile_moc_single(const std::string &src);
...
CBuild& CBuild::moc_src(std::vector<std::string> moc_src) {
	m_moc_src.insert(m_moc_src.end(), moc_src.begin(), moc_src.end());
	return *this;
}
...
//Building moc files
CBuild& CBuild::build_moc() {
	for (auto moc_src : m_moc_src) {
		if (contains(moc_src, ".h")) {
			std::string obj = moc_src;
			replace(obj,".h","");
			std::string moc_cmd = "moc -o " + obj + ".moc.cpp " + moc_src;
			LOG(moc_cmd.c_str());

			int status = std::system(moc_cmd.c_str());
			if (status) {
				LOG("MOC generation failed for: %s", moc_src.c_str());
				exit(1);
			}
		}
	}
...
CBuild& CBuild::compile() {
	for(auto src : m_moc_src){
		compile_moc_single(src);
	}
    // Compile only regular .cpp files
    for (auto src : m_src) {
...
CBuild& CBuild::build() {
    ...
    std::string cmd = m_cc + " "
                    + flags
                    + " -o " + m_out_dir + (m_out_dir.length() ? "/" : "") + m_out_file + " "
                    + objs + " "
                    + lib_paths + " "
                    + libs;
    ...
    LOG("Successfully built: %s", m_out_file.c_str());
...
bool CBuild::compile_moc_single(const std::string& src) {
	std::string flags = vec_join(m_flags);
	std::string inc_paths = vec_join(m_inc_paths, "-I");

	std::string obj = src;
	std::string moc_src = src;
	if (contains(obj, ".h"))
		replace(obj, ".h", "_moc.o");
	if (contains(moc_src, ".h"))
